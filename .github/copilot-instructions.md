...
| `pyrnnoise` | >=0.4.3 | Real-time RNNoise mic noise suppression (replaces `noisereduce` which produced robot-voice artifacts). Uses the low-level C bindings (`pyrnnoise.rnnoise.process_frame`) directly — we do NOT use the high-level `RNNoise` Python class because it routes audio through libav graphs which is too heavy for the audio callback. ~0.5% CPU. |
