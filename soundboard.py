...
class SoundboardApp:
    ...
    def _create_ui(self):
        ...
        board_frame = ttk.LabelFrame(
            main_frame, text="Soundboard (Right-click to configure)", padding=10
        )
        ...
        ttk.Label(main_frame, textvariable=self.status_var, relief=tk.SUNKEN, anchor="w").pack(
            fill=tk.X, pady=(10, 0)
        )
    ...
    def _configure_slot(self, slot_idx):
        ...
        tk.Button(btn_frame, text="Save", command=save, bg="#43B581", fg="white", width=10).pack(
            side=tk.LEFT, padx=5
        )
        tk.Button(btn_frame, text="Clear", command=clear, bg="#F04747", fg="white", width=10).pack(
            side=tk.LEFT, padx=5
        )
        tk.Button(
            btn_frame, text="Cancel", command=dialog.destroy, bg="#40444B", fg="white", width=10
        ).pack(side=tk.LEFT, padx=5)
